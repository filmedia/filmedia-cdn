...
// private helpers
    ...
    var processData = function($this, data, term)
    {
        ...
        {
            $('<li><a href="'+'https://filmedia.alwaysdata.net/films/'+results[i].originalDataItem.slug+'">' + config.renderFunction(results[i].dataItem, results[i].topMatch, results[i].originalDataItem, config) + '</li>')
        ...
        if ((config.noResultsMessage !== undefined) && (results.length == 0))
            $list.append($('<li class="' + config.noResultsClass + '">' + config.noResultsMessage + '</a></li>'));
...
// default functions
    var defaultRenderFunction = function(dataItem, topMatch, originalDataItem, config)
    {
        if ((topMatch === config.nameField) || (topMatch === null))
            ...
            return '<p class="title">' + dataItem[config.nameField] + '</p>' +
                   '<p class="matchRow">';
                   // +'<span class="matchedField">' + topMatch + '</span>: ' +
                   //      dataItem[topMatch] + '</p>';
